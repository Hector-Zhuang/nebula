...
export default function SampleMiniApp(props) {
  ...
  const buildNebulaUrl = (action) =>
    `nebula://sample-miniapp/demo?action=${action}&from=${encodeURIComponent(routePath)}&stack=${nextStack}&ts=${Date.now()}`;
  ...
  return (
    <ScrollView contentContainerStyle={styles.container}>
      ...
      <View style={styles.buttonWrap}><Button title="navigateTo (push + params)" onPress={() => run('navigateTo', () => MiniAppAPI.navigateTo(buildNebulaUrl('navigateTo')))} /></View>
      <View style={styles.buttonWrap}><Button title="redirectTo (replace current)" onPress={() => run('redirectTo', () => MiniAppAPI.redirectTo(buildNebulaUrl('redirectTo')))} /></View>
      <View style={styles.buttonWrap}><Button title="reLaunch (reset stack)" onPress={() => run('reLaunch', () => MiniAppAPI.reLaunch(buildNebulaUrl('reLaunch')))} /></View>
      <View style={styles.buttonWrap}><Button title="navigateBack delta=1" onPress={() => run('navigateBack(1)', () => MiniAppAPI.navigateBack(1))} /></View>
      <View style={styles.buttonWrap}><Button title="navigateBack delta=2" onPress={() => run('navigateBack(2)', () => MiniAppAPI.navigateBack(2))} /></View>
      <View style={styles.buttonWrap}><Button title="wx.navigateTo wrapper" onPress={() => run('wx.navigateTo', () => wx.navigateTo({ url: buildNebulaUrl('wx.navigateTo') }))} /></View>
      <View style={styles.buttonWrap}><Button title="wx.showToast + param echo" onPress={() => run('wx.showToast', () => wx.showToast({ title: `from ${routePath}` }))} /></View>
