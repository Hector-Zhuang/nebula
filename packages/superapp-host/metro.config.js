...
const projectRoot = __dirname; // packages/superapp-host/
  ...
  server: {
    enhanceMiddleware: (middleware) => {
      return (req, res, next) => {
        ...
        if (match) {
          const appId = match[1];
          const manifestPath = path.join(workspaceRoot, 'packages', appId, 'app.json');
        ...
        if (req.url === '/app.json') {
          const manifestPath = path.join(workspaceRoot, 'packages', 'sample-miniapp', 'app.json');
          if (fs.existsSync(manifestPath)) {
            res.setHeader('Content-Type', 'application/json');
            res.end(fs.readFileSync(manifestPath));
            return;
