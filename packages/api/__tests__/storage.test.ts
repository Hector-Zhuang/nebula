...
describe('storage', () => {
  describe('setStorage', () => {
    test('should set value into storage', async () => {
      ...
      const getData = storage.getString(key) || ''
  ...
  describe('getStorageInfo', () => {
    test('获得正确的StorageInfo', async () => {
      ...
      storage.set(key1, JSON.stringify(data))
      storage.set(key2, JSON.stringify(data))
  ...
  describe('clearStorage', () => {
    test('clearStorage能清楚所有的Storage', async () => {
      ...
      const getData1 = storage.getString(key1) || ''
      const getData2 = storage.getString(key2) || ''
