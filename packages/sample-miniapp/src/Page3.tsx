...
export default function Page3(props) {
  ...
  const testStackLimit = async () => {
    try {
      const url = `/page3?depth=${stackTestCount + 1}`;
      await Miniapp.navigateTo(url);
      setStackTestCount(stackTestCount + 1);
      setResult(`Stack push #${stackTestCount + 1} succeeded`);
    } catch (error) {
      const message = error?.message ?? String(error);
      setResult(
        `Stack limit test: FAILED after ${stackTestCount} pushes - ${message}`,
      );
    }
  };
      ...
      <View style={styles.card}>
        ...
        <View style={styles.buttonWrap}>
          <Button
            ...
            onPress={() =>
              run('navigateTo', () =>
                Miniapp.navigateTo(`/page1?from=page3&ts=${Date.now()}`),
              )
            }
            color="#3b82f6"
          />
        ...
        <View style={styles.buttonWrap}>
          <Button
            ...
            onPress={() =>
              run('navigateTo', () =>
                Miniapp.navigateTo(`/page2?from=page3&ts=${Date.now()}`),
              )
            }
            color="#8b5cf6"
          />
        ...
        <View style={styles.buttonWrap}>
          <Button
            ...
            onPress={() =>
              run('navigateBack(3)', () => Miniapp.navigateBack(3))
            }
            color="#059669"
          />
        </View>
      ...
      <View style={styles.card}>
        ...
        <View style={styles.buttonWrap}>
          <Button
            title="showToast"
            onPress={() =>
              run('showToast', () => Miniapp.showToast('Page 3 here!'))
            }
            color="#06b6d4"
          />
        </View>
