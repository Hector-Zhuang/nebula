...
class SocketTask {
  ...
  close (opts?: SocketTask.CloseOption): void {
    const {
      code = 1000,
  ...
  onOpen (func?: SocketTask.OnOpenCallback): void {
    this.ws.onopen = () => {
      func && func({
        header: {}
      })
  ...
  onMessage (func?: SocketTask.OnMessageCallback): void {
  ...
  onClose (func?: SocketTask.OnCloseCallback): void {
    this.ws.onclose = () => {
      this._destroyWhenClose && this._destroyWhenClose()
  ...
  onError (func?: SocketTask.OnErrorCallback): void {
    this.ws.onerror = () => {
      func && func({
